...
from transformers import CLIPTextModel, AutoProcessor, CLIPVisionModel
...
class CLIP_TEXT_ENCODER(nn.Module):
    def __init__(self):
        ...
        print('Load pretrained CLIPtextmodel')
        self.define_module(clip_text)
        self.init_weights()

    def define_module(self, clip_text):
        self.cliptextmodel= clip_text
    ...
    def forward(self, captions):
        # Iterate over the values of the captions dictionary
        for key, value in captions.items():
            captions[key] = value.to(self.cliptextmodel.device)
        outputs = self.cliptextmodel(**captions)
        words_emb = self.linear(outputs.last_hidden_state)
        words_emb =words_emb.transpose(1, 2)
        sent_emb = self.linear(outputs.pooler_output)	
...
class CLIP_VISION_ENCODER(nn.Module):
    def __init__(self, nef):
        ...
        for param in clip_vision.parameters():
            param.requires_grad = False
        print('Load pretrained CLIPVisionModel')

        processor = AutoProcessor.from_pretrained("openai/clip-vit-base-patch32")
        print('Load pretrained AutoProcessor')

        self.define_module(clip_vision,processor)
        self.init_trainable_weights()

    def define_module(self, clip_vision, processor):
        self.autoprocessor= processor    
        self.clipvisionmodel= clip_vision
        self.up_sample = nn.Upsample(size=[17,17], mode='nearest')
        self.emb_features = conv1x1(150, self.nef)
        self.emb_code = nn.Linear(768, self.nef)

    def init_trainable_weights(self):
        initrange = 0.1
        self.emb_features.weight.data.uniform_(-initrange, initrange)
        self.emb_code.weight.data.uniform_(-initrange, initrange)

    def forward(self, x):
        x = (x + 1.0) / 2.0
        inputs = self.autoprocessor(images=x, return_tensors="pt")
        outputs = self.clipvisionmodel(**inputs.to(self.clipvisionmodel.device))
        last_hidden_state = outputs.last_hidden_state.view(-1,150,16,16)
        last_hidden_state = self.up_sample(last_hidden_state)
        features = self.emb_features(last_hidden_state)
        code = self.emb_code(outputs.pooler_output)
